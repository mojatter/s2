...
func TestS3Action(t *testing.T) {
	testCases := []struct {
		...
		{
			caseName:     "list multipart uploads with trailing slash",
			method:       http.MethodGet,
			url:          "/mybucket/?uploads",
			bucket:       "mybucket",
			wantAction:   "s3:ListBucketMultipartUploads",
		...
		},
		{
			caseName:     "list parts",
			method:       http.MethodGet,
			url:          "/mybucket/a/b.txt?uploadId=abc",
			bucket:       "mybucket",
			key:          "a/b.txt",
			wantAction:   "s3:ListMultipartUploadParts",
			wantResource: "arn:aws:s3:::mybucket/a/b.txt",
		},
		{
			caseName:     "head with an upload id is still a head object",
			method:       http.MethodHead,
			url:          "/mybucket/a/b.txt?uploadId=abc",
			bucket:       "mybucket",
			key:          "a/b.txt",
