...
On `osfs` and `memfs` the element `.meta` is reserved at every depth: it holds the JSON metadata file of another object, so neither `.meta/a.txt` nor `docs/.meta/a.txt` is an object of its own and both are refused. Every depth, because each directory keeps its own objects' metadata files — `docs/a.txt` is described by `docs/.meta/a.txt` — so the storage, any `Sub` of it and a `DirFS` opened on `docs` all find the same one, and a listing hides the directory wherever it appears — a name reaching through one would be stored and read but never listed. `Sub` refuses it too, so no caller-supplied prefix reaches the metadata files; s2-server keeps its own per-bucket state in `.buckets/`, an ordinary prefix on every backend, rather than reaching in.

Before v0.20.0 a nested object's metadata file lived under the `.meta` of the storage that wrote it, so `docs/a.txt` written through the root was described by `.meta/docs/a.txt` and a `Sub` of `docs` did not see it. `fs.MigrateMeta` moves every such metadata file beside its object in one walk: it treats any directory inside a `.meta` as the old layout, since the new one holds only files there, and keeps the newer of two for the same object. A file under a `.meta` whose object is gone stays where it is, as do one for a key holding `.meta` and one that does not parse as a metadata file: either may be a key a pre-v0.18.1 server stored rather than a metadata file, and s2 cannot tell them apart. A directory kept this way under the root's `.meta`, such as `.meta/photos/`, takes the place of the metadata file a top-level object `photos` would need, so writing that object fails, without changing anything, until the directory is removed. A directory at an object's new location, which another view can leave behind, is removed when empty and otherwise left alone and reported like a blocking file. Run it while nothing writes to the storage; it is safe to rerun. Until a root is migrated, a storage still reads the metadata files it wrote itself under the old layout, and its next write to the object moves them; only a metadata file written through a different view, such as the root reading what a `Sub` wrote, stays invisible until `MigrateMeta` runs. That fallback is removed in v1.0.0. s2-server reaches every bucket through one `Sub`, so its buckets keep reading through the fallback without a migration; a migration command ships before v1.0.0.

A regular file named `.meta` — a key a pre-v0.18.1 server could store — takes the place of the directory its neighbours' metadata files need. The objects beside it stay readable, through the old metadata files `MigrateMeta` leaves in place, but a write to one fails before touching the object. `MigrateMeta` reports them as `fs.ErrMetaBlocked` after migrating everything else, and a later run moves them once the file is gone. Removing the file, on `osfs` from the root directory, clears it. A key a pre-v0.18.1 server stored as `docs/.meta/a.txt` is now read as the metadata file of `docs/a.txt`, as `.meta/a.txt` already was for `a.txt`. When it does not parse as one, reading `docs/a.txt` fails with an error naming `docs/.meta/a.txt`. `MigrateMeta` reports it with `fs.ErrMetaBlocked` only when a legacy metadata file for `docs/a.txt` is waiting to move there, and then touches neither file; with nothing to move, only the read error names it. Either way, fixing or removing the file clears it. A write to `docs/a.txt` replaces such a file with its metadata file, as a write through a `Sub` of `docs` already did before v0.20.0, so move it away first if it matters.
